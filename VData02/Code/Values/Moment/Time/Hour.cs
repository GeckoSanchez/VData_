namespace VData02.Values
{
	...
	public class Hour : BaseNumber<int>, IEquatable<Hour?>
	{
		[DefaultValue(0)]
		...
		public Hour(int data) : base(data)
		{
			...
			try
			{
				Data = new BaseNumber<int>(data, DateTime.MinValue.Hour, DateTime.MaxValue.Hour).Data;
			}
			catch (Exception ex)
			{
		...
		public Hour(DateTime data) : this(data.Hour) => Log.Event(new StackFrame(true));

		/// <param name="data">A <see cref="Time"/> time object</param>
		/// <inheritdoc cref="Hour(DateTime)"/>
		public Hour(Time data) : this(data.Data.Hour) => Log.Event(new StackFrame(true));

		/// <param name="data">A <see cref="Moment"/> moment object</param>
		/// <inheritdoc cref="Hour(DateTime)"/>
		public Hour(Moment data) : this(data.Data.Hour) => Log.Event(new StackFrame(true));

		/// <param name="data">A <see cref="TimeOnly"/> time object</param>
		/// <inheritdoc cref="Hour(DateTime)"/>
		public Hour(TimeOnly data) : this(data.Hour) => Log.Event(new StackFrame(true));

		/// <param name="data">A <see cref="Hour"/> hour data</param>
		/// <inheritdoc cref="Hour(DateTime)"/>
		[SelfConstructor]
		public Hour(Hour data) : this(data.Data) => Log.Event(new StackFrame(true));

		/// <summary>
		/// Empty constructor for the <see cref="Hour"/> class
		/// </summary>
		[EmptyConstructor]
		public Hour() : this(DateTime.MinValue.Hour) => Log.Event(new StackFrame(true));
		...
		/// <inheritdoc cref="Base{int}.op_Equality"/>
		/// <exception cref="TimeException"/>
		public static bool operator ==(Hour? left, int right)
		{
			bool Out;
