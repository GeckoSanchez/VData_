namespace VData02.IDs
{
	...
	{
		/// <summary>
		/// [PROTECTED] Main constructor for the <see cref="ObjectID"/> class
		/// </summary>
		/// <param name="data">A <see cref="UInt128"/> data value</param>
		[JsonConstructor, MainConstructor]
		protected ObjectID([NotNull] UInt128 data) : base(data) => Log.Event(new StackFrame(true));

		/// <summary>
		/// Constructor for the <see cref="ObjectID"/> clas
		/// </summary>
		/// <param name="kind">A <see cref="ObjectKind"/> kind</param>
		/// <exception cref="IDException"/>
		public ObjectID([NotNull] ObjectKind kind) : this(UInt128.Zero)
		{
		...
		/// <param name="kind">A <see cref="BaseKind{ObjectKind}"/> data object</param>
		/// <inheritdoc cref="ObjectID(ObjectKind)"/>
		public ObjectID([NotNull] BaseKind<ObjectKind> kind) : this(UInt128.Zero)
		{
			var sf = new StackFrame(true);
			Log.Event(sf);
		...
		/// <inheritdoc cref="ObjectID(ObjectKind)"/>
		public ObjectID([NotNull] ObjectKind kind, [NotNull] ulong subID) : this(UInt128.Zero)
		{
			var sf = new StackFrame(true);
			Log.Event(sf);
		...
		/// <inheritdoc cref="ObjectID(ObjectKind)"/>
		public ObjectID([NotNull] ObjectKind kind, [NotNull] long subID) : this(UInt128.Zero)
		{
			var sf = new StackFrame(true);
			Log.Event(sf);
		...
		/// <param name="kind">A <see cref="BaseKind{ObjectKind}"/> kind object</param>
		/// <inheritdoc cref="ObjectID(ObjectKind, ulong)"/>
		public ObjectID([NotNull] BaseKind<ObjectKind> kind, [NotNull] ulong subID) : this(UInt128.Zero)
		{
			var sf = new StackFrame(true);
			Log.Event(sf);
		...
		public ObjectID([NotNull] BaseKind<ObjectKind> kind, [NotNull] long subID) : this(UInt128.Zero)
		{
			var sf = new StackFrame(true);
			Log.Event(sf);
		...
		/// <param name="kind">A <see cref="ObjectType"/> kind object</param>
		/// <inheritdoc cref="ObjectID(ObjectKind, ulong)"/>
		public ObjectID([NotNull] ObjectType kind, [NotNull] ulong subID) : this(UInt128.Zero)
		{
			var sf = new StackFrame(true);
			Log.Event(sf);
		...
		/// <param name="kind">A <see cref="ObjectType"/> kind object</param>
		/// <inheritdoc cref="ObjectID(ObjectKind, long)"/>
		public ObjectID([NotNull] ObjectType kind, [NotNull] long subID) : this(UInt128.Zero)
		{
			var sf = new StackFrame(true);
			Log.Event(sf);
		...
		/// <exception cref="IDException"/>
		[EmptyConstructor]
		public ObjectID() : this(UInt128.Zero)
		{
			var sf = new StackFrame(true);
			Log.Event(sf);
			...
			try
			{
				if (!UInt128.TryParse($"{Def.ObjectKind.GetType().GetHashCode():X8}{Def.ObjectKind.GetHashCode():X8}{0:X16}", NumberStyles.HexNumber, null, out UInt128 data))
					throw new Exception($"This ID could not be generated");
		...
		/// <summary>
		/// Function to extract the sub-ID of the current <see cref="ObjectID"/> object
		/// </summary>
